...
tkWindow.title('Cryptocurrency Bot')
...
class Trading_Gui:
    def __init__(self, top=None):
        ...
        self.totalPnl.configure(foreground="grey")
        ...
        self.totalPnl.configure(selectforeground="white")
        self.totalPnl.configure(takefocus="0")
    ...
    def stopTrade(self):
        ...
        try:
            ...
            opPos = []
        if len(opPos) > 0:
            messagebox.showinfo("Warning", "There are in a opened position. Close all before stopping all trading!")
        else:
            self.startTrading = False
        ...
        if n > self.numberOfExecutions:
            ...
            for row in newExecutions:
                self.Scrolledtreeview1.insert('', 'end', text=str(k), \
                                              values=(str(row[0]).split(" ")[-1], row[1], row[3], round(row[2], 2),
                                                      round(row[4], 3)))
                k = k + 1
    ...
    def main(self):
        ...
        try:
            ...
            messagebox.showinfo("Error", "Wrong StopLoss input. Must be an numeric.")
            error = True
        try:
        ...
        try:
            ...
            if timePosEdge < 0:
                messagebox.showinfo("Error", "Wrong Maintenance input. Must be an integer>=0.")
                timePosEdge = 1
        except:
            messagebox.showinfo("Error", "Wrong Maintenance input. Must be an integer.")
            error = True
        if not error:
                ...
                try:
                    ...
                    if self.showChart:
                        self.liveChartObject.botLiveChart(x=histDataUpdated.Date, y=histDataUpdated.Price, \
                                                          tradesLive=executions,
                                                          lastTradePnl=self.strategy.lastTradePnl, \
                                                          totalPnl=self.strategy.totalPnl,
